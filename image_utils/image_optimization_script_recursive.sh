...
find . -iname "*.png" -exec ls -lh {} \;

# Store the total size of PNG files before optimization
size_before=$(find . -iname "*.png" -exec du -ch {} + | grep total$ | awk '{print $1}')

# Loop through all png images in the specified directory and its subdirectories
...
size_after=$(find . -iname "*.png" -exec du -ch {} + | grep total$ | awk '{print $1}')
